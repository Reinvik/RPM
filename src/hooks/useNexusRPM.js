      ...
      try {
        ...
        const mechanics = (mechanicsData || []).map(m => {
          ...
          return {
            ...
            prestamos: Number(m.prestamos) || 0,
            descuentos: Number(m.descuentos) || 0,
            asistencia: Number(m.asistencia) || 0,
            vacaciones_acumuladas: Number(m.vacaciones_acumuladas) || 0,
            vacaciones_tomadas: Number(m.vacaciones_tomadas) || 0,
            fecha_ingreso: m.fecha_ingreso || '',
            rut: m.rut || '',
            cargo: m.cargo || '',
            bonos: Number(m.bonos) || 0,
            mo_generada,
            insumos_generados,
            tipo: m.tipo || 'Fijo',
            status: 'pending'
